...
func doWork(wg *sync.WaitGroup, done <-chan bool, data <-chan any, id int) {
	defer wg.Done()
...
func main() {
	...
	for {
		...
		case <-sigChan: // ждем получения сигнала os.Interrupt
			// бродкастим воркерам о завершении с помощью закрытия канала done
			close(done)
